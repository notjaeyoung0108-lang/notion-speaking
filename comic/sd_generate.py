...
def compose_webtoon_panel(image, text: str = "", face_bbox: tuple | None = None,
    ...
    if text:
        ...
        scale, lines, line_h = _fit_bubble(
            text, asset, font, tmp_draw, sw, sh,
        ...
        while line_h * len(lines) > tb_h0 and fsz > P.BUBBLE_MIN_FONT:
            ...
            lines = _wrap_rich_text(text, font, italic_font, tb_w0, tmp_draw, target_sentence)
    ...
    draw = ImageDraw.Draw(canvas)
    black = (0, 0, 0)
    draw.rectangle([ox, oy, ox + sw - 1, oy + sh - 1], outline=black, width=bd)
    draw.rectangle([0, 0, bd - 1, ch - 1], fill=gutter)           # left rail (full height)
    draw.rectangle([cw - bd, 0, cw - 1, ch - 1], fill=gutter)     # right rail (full height)
    draw.rectangle([bd, 0, bd + 1, ch - 1], fill=black)
    draw.rectangle([cw - bd - 2, 0, cw - bd - 1, ch - 1], fill=black)
    ...
    tb_w = tb_r - tb_l
    tb_h = tb_b - tb_t
    lines = _wrap_rich_text(text, font, italic_font, max(10, tb_w), draw, target_sentence)
    while line_h * len(lines) > tb_h and font.size > P.BUBBLE_MIN_FONT:
        font = ImageFont.truetype(P.BUBBLE_FONT, max(P.BUBBLE_MIN_FONT, font.size - 2))
        italic_font = _load_italic_font(font, P.BUBBLE_FONT)
        line_h = int(font.size * P.BUBBLE_LINE_H)
    ...
    draw.rectangle([0, 0, bd - 1, ch - 1], fill=gutter)
    draw.rectangle([cw - bd, 0, cw - 1, ch - 1], fill=gutter)
    draw.rectangle([bd, 0, bd + 1, ch - 1], fill=black)
    draw.rectangle([cw - bd - 2, 0, cw - bd - 1, ch - 1], fill=black)
    print(f"  💬 webtoon bubble corner={key} (lift={WEBTOON_LIFT_RATIO})")
    return canvas
...
class Generator:
    ...
    def _render_panels(self, panels_cfg: list[dict], seed: int,
        ...
        for idx, pcfg in enumerate(panels_cfg):
            ...
            bubble_text = "" if is_object_panel else pcfg.get("bubble", "")
            if bubble_text:
